...
1. **Get the planned distance.** Ask Karan if he hasn't said.
...
When Karan names a time rather than "now" ("this evening", "tomorrow
...
overrides the recommendation when Karan has already decided.
...
- Trailing load and rest are measured against the clock, so a stale cache reads
  as more rest than Karan actually had. `cache.py status` flags that before it
  matters.
